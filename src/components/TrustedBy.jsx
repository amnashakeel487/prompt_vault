  ...
  return (
    <section className="border-y border-line bg-surface/30">
      <div className="section-pad py-6 sm:py-8 text-center">
        <p className="text-[10px] sm:text-[11px] font-mono uppercase tracking-widest text-ink-faint">
        ...
        <div className="mt-4 sm:mt-5 flex flex-wrap items-center justify-center gap-x-5 sm:gap-x-8 gap-y-3 sm:gap-y-4 md:gap-x-12">
          {TEAMS.map((name, i) => (
            <motion.span
              ...
              className="font-display text-xs sm:text-sm font-semibold tracking-tight text-ink-faint transition-colors hover:text-ink-muted select-none"
            >
              {name}
            </motion.span>
          ))}
