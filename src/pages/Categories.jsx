  ...
  return (
    <section className="section-pad py-8 sm:py-14">
      ...
      <div className="mb-8 sm:mb-10">
        <h1 className="font-display text-2xl sm:text-3xl font-semibold text-ink">Categories</h1>
        <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-ink-muted">Every prompt organized by use case.</p>
      </div>

      {loading ? (
        <div className="grid gap-3.5 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="glass-card h-20 sm:h-24 animate-pulse" />
          ))}
        </div>
